...
class Inception_v3(torch.nn.Module):
    def __init__(self, class_num, separate_abs_sign, pretrained=False, use_new_func=True, freeze_key="none"): # freeze_key = ["none", "without_bn", "all"]
        super().__init__()
        ...
        self.dense = torch.nn.Linear(in_features=self.in_features, out_features=self.self.out_features, bias=True)
        self.relu = torch.nn.ReLU(inplace=True)
        self.finetune = True if freeze_key == "all" else False
        
        if self.separate_abs_sign == 3:
            self.predictions = torch.nn.Linear(in_features=self.out_features, out_features=class_num, bias=True)
            self.softmax = torch.nn.Softmax(dim=1)
        elif self.separate_abs_sign == 1:
            self.predictions_abs = torch.nn.Linear(in_features=self.out_features, out_features=1, bias=True)
            self.predictions_sign = torch.nn.Linear(in_features=self.out_features, out_features=1, bias=True)
            self.sigmoid = torch.nn.Sigmoid()
        elif self.separate_abs_sign == 0:
            self.predictions = torch.nn.Linear(in_features=self.out_features, out_features=class_num, bias=True)
        else:
            self.predictions = torch.nn.Linear(in_features=self.out_features, out_features=1, bias=True)
...
class VGG19_BN(torch.nn.Module):
    def __init__(self, class_num, separate_abs_sign, pretrained=False, use_new_func=True, freeze_key="none"): # freeze_key = ["none", "without_bn", "all"]
        ...
        self.out_features = 1024
        self.backbone.classifier[-1] = torch.nn.Linear(in_features=self.in_features, out_features=self.out_features, bias=True)
        self.relu = torch.nn.ReLU(inplace=True)
        self.finetune = True if freeze_key == "all" else False
...
class EfficientNet(torch.nn.Module):
    def __init__(self, network, class_num, separate_abs_sign, pretrained=False, use_new_func=True, freeze_key="none"): # freeze_key = ["none", "without_bn", "all"]
        super().__init__()
...
def create_model_full_scratch(network, num_classes, separate_abs_sign, trainable_only_bn=False):
    
    freeze_key = "without_bn" if trainable_only_bn == True else "none"
    
    if network == "InceptionV3":
        model = Inception_v3(num_classes, separate_abs_sign, pretrained=True, use_new_func=True, freeze_key=freeze_key)
    elif network == "VGG19_bn":
        model = VGG19_BN(num_classes, separate_abs_sign, pretrained=True, use_new_func=True, freeze_key=freeze_key)
    elif network == "ResNet50":
        model = ResNet50(num_classes, separate_abs_sign, pretrained=True, use_new_func=True, freeze_key=freeze_key)
    elif "EfficientNet" in network:
        model = EfficientNet(network, num_classes, separate_abs_sign, pretrained=True, use_new_func=True, freeze_key=freeze_key)
    
    return model

def create_model_fine_tuning(network, num_classes, separate_abs_sign):
    
    if network == "InceptionV3":
        model = Inception_v3(num_classes, separate_abs_sign, pretrained=True, use_new_func=True, freeze_key="all")
    elif network == "VGG19_bn":
        model = VGG19_BN(num_classes, separate_abs_sign, pretrained=True, use_new_func=True, freeze_key="all")
    elif network == "ResNet50":
        model = ResNet50(num_classes, separate_abs_sign, pretrained=True, use_new_func=True, freeze_key="all")
    elif "EfficientNet" in network:
        model = EfficientNet(network, num_classes, separate_abs_sign, pretrained=True, use_new_func=True, freeze_key="all")
